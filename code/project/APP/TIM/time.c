...
void time_init()
{
	...
	//定时器的计算澹澹:((1+TIM_Prescaler )/72M)*(1+TIM_Period )预分频最大为65535
	TIM_ClearITPendingBit(TIM3,TIM_IT_Update);//清除TIMx的中断待处理位:TIM 中断源
	TIM_TimeBaseInitStructure.TIM_Period = 39;//设置自动重装载寄存器周期的值
	TIM_TimeBaseInitStructure.TIM_Prescaler = 35999;//设置用来作为TIMx时钟频率预分频值，0.5ms
